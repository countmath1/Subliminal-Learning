...
CONFIG="${1:-configs/exp_reason.yaml}"
if [ ! -f "$CONFIG" ]; then
...
if [ ! -d "$MODEL_CACHE" ]; then
  ...
  echo "  export HF_HOME=\$HOME/hf_cache" >&2
  echo "  hf download $MODEL" >&2
  exit 3
fi

python src/exp_reason_infer.py --config "$CONFIG" --out "$OUT_DIR"
echo "OUT_DIR=$OUT_DIR" > "logs/exp_reason-${SLURM_JOB_ID}.outdir"
